...
\end{align}
    ...
    Para continuar com esta conta precisamos de algumas séries. Partindo de
...
\end{align}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}24}]:} \PY{n}{x}\PY{p}{,} \PY{n}{j} \PY{o}{=} \PY{n}{symbols}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{x j}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
...
\(e^{-\beta\hbar\omega} \in (0,1)\)
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}25}]:} \PY{n}{Sum}\PY{p}{(}\PY{n}{j}\PY{o}{*}\PY{n}{x}\PY{o}{*}\PY{o}{*}\PY{n}{j}\PY{p}{,}\PY{p}{(}\PY{n}{j}\PY{p}{,}\PY{l+m+mi}{0}\PY{p}{,}\PY{n}{oo}\PY{p}{)}\PY{p}{)}\PY{o}{.}\PY{n}{doit}\PY{p}{(}\PY{p}{)}
\end{Verbatim}
...
\end{align}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}26}]:} \PY{c+c1}{\PYZsh{} não são todas as contas são facilmente feitas com a ajuda do software}
...
\texttt{\color{outcolor}Out[{\color{outcolor}26}]:}
    ...
    $$\sum_{j=0}^{\infty} \left(1 - e^{- x}\right) \left(j + 0.5\right) e^{- j x}$$

    

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}27}]:} \PY{c+c1}{\PYZsh{} Derivadas}
...
\end{align}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}28}]:} \PY{n}{Sum}\PY{p}{(}\PY{p}{(}\PY{n}{j}\PY{o}{*}\PY{o}{*}\PY{l+m+mi}{2}\PY{p}{)}\PY{o}{*}\PY{n}{x}\PY{o}{*}\PY{o}{*}\PY{n}{j}\PY{p}{,}\PY{p}{(}\PY{n}{j}\PY{p}{,}\PY{l+m+mi}{0}\PY{p}{,}\PY{n}{oo}\PY{p}{)}\PY{p}{)}\PY{o}{.}\PY{n}{doit}\PY{p}{(}\PY{p}{)}
\end{Verbatim}

\texttt{\color{outcolor}Out[{\color{outcolor}28}]:}
    
    $$\begin{cases} \frac{x \left(- x - 1\right)}{\left(x - 1\right)^{3}} & \text{for}\: x < 1 \\\sum_{j=0}^{\infty} j^{2} x^{j} & \text{otherwise} \end{cases}$$
...
}{Integral Gaussiana G = \textbackslash{}int\_\{-\textbackslash{}infty\}\^{}\{\textbackslash{}infty\}e\^{}\{-ax\^{}\{2\}\}dx = \textbackslash{}sqrt\{\textbackslash{}frac\{\textbackslash{}pi\}\{a\}\}  }}\label{integral-gaussiana-g-int_-inftyinftye-ax2dx-sqrtfracpia}
...
\(x\in(-\infty,\infty)\), \(y\in(-\infty,\infty)\), \(r\in[0,\infty)\),
\(\theta\in[0,2\pi)\). Assim
...
\end{align}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}30}]:} \PY{n}{x}\PY{p}{,} \PY{n}{a} \PY{o}{=} \PY{n}{symbols}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{x a}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
...
\texttt{\color{outcolor}Out[{\color{outcolor}30}]:}
    ...
    $$\begin{cases} \frac{\sqrt{\pi}}{\sqrt{a}} & \text{for}\: \left|{\operatorname{periodic_{argument}}{\left (a,\infty \right )}}\right| \leq \frac{\pi}{2} \\\int_{-\infty}^{\infty} e^{- a x^{2}}\, dx & \text{otherwise} \end{cases}$$
...
\end{align}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}20}]:} \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Distribuição de probabilidades Gaussiana}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
...
Distribuição de probabilidades Gaussiana
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
        ...
        <ipython-input-20-b38911ab9eb2> in <module>()
         38 axes = plt.gca()
         39 axes.set\_xlim([-10,10])
    ---> 40 axes.set\_ylim([-0.1,])
         41 plt.show()


        \textasciitilde{}/anaconda3/lib/python3.6/site-packages/matplotlib/axes/\_base.py in set\_ylim(self, bottom, top, emit, auto, **kw)
       3267 
       3268         if top is None and iterable(bottom):
    -> 3269             bottom, top = bottom
       3270 
       3271         bottom = self.\_validate\_converted\_limits(bottom, self.convert\_yunits)
...
graficar a distribuição de probabilidades desses valores médios.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}21}]:} \PY{c+c1}{\PYZsh{} M jogos, cada um com N roladas do dado}
         \PY{k+kn}{from} \PY{n+nn}{numpy} \PY{k}{import} \PY{n}{random}\PY{p}{;}  \PY{n}{random}\PY{o}{.}\PY{n}{seed}\PY{p}{(}\PY{p}{)}
         \PY{k+kn}{from} \PY{n+nn}{scipy}\PY{n+nn}{.}\PY{n+nn}{stats} \PY{k}{import} \PY{n}{norm}
         \PY{n}{M} \PY{o}{=} \PY{l+m+mi}{1000}\PY{p}{;}  \PY{n}{N} \PY{o}{=} \PY{l+m+mi}{2}\PY{o}{*}\PY{o}{*}\PY{l+m+mi}{3}
...
A covariância é uma de correlação entre os valores de duas variáveis
aleatórias e é definida por

\begin{equation}
Cov(X,Y)=\langle XY\rangle-\langle X\rangle\langle Y\rangle.
\end{equation}

Para calcular o valor médio do produto \(XY\) precisamos da distribuição
conjunta de probabilidades, ou seja, devemos conhecer a frequência
relativa com que todos os pares de valores de \(X\) e \(Y\) são obtidos.
Denotando \(Pr(X=X_{j},Y=Y_{k})=p_{j,k}\) teremos que

\begin{equation}
\langle XY\rangle = \sum_{j,k}X_{j}Y_{k}p_{j,k}.
\end{equation}

\textbf{Exercício:} Mostre que se a distribuição de probabilidades
conjunta é o produto das distribuições de probabilidade condicionais,
i.e., se \(p_{j,k}=p_{j}p_{k}\), então \(Cov(X,Y)=0\) e não existe
correlação entre essas variáveis aleatórias.

\emph{OBS.} As distribuições de probabilidade marginais são obtidas
somando-se todas as frequências relativas nas quais aparece um certo
valor da variável considerada. Por exemplo,
